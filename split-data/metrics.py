...
def _mask_valid(obs: DataArray, sim: DataArray) -> Tuple[DataArray, DataArray]:
    ...
    try:
        ...
        obs = obs[idx.values[:,0]]
        sim = sim[idx.values[:,0]]
...
def kge(obs: DataArray, sim: DataArray, weights: List[float] = [1., 1., 1.]) -> float:
    ...
    try:
        ...
        r, _ = stats.pearsonr(obs.values[:,0], sim.values[:,0])
...
def calculate_all_metrics_for_frequency_analysis(analysis_dict, obs, lstm, nwm):

    for metric in get_available_metrics():

        analysis_dict[metric]['ris'].append(recurrance_interval)
        
        if metric == 'NSE':
            lstm_score = nse(obs,lstm)
            nwm_score = nse(obs,nwm)
        if metric == 'MSE':
            lstm_score = mse(obs,lstm)
            nwm_score = mse(obs,nwm)
        if metric == 'RMSE':
            lstm_score = rmse(obs,lstm)
            nwm_score = rmse(obs,nwm)
        if metric == 'KGE':
            lstm_score = kge(obs,lstm)
            nwm_score = kge(obs,nwm)
        if metric == 'Alpha-NSE':
            lstm_score = alpha_nse(obs,lstm)
            nwm_score = alpha_nse(obs,nwm)
        if metric == 'Beta-NSE':
            lstm_score = beta_nse(obs,lstm)
            nwm_score = beta_nse(obs,nwm)
        if metric == 'Pearson-r':
            lstm_score = pearsonr(obs,lstm)
            nwm_score = pearsonr(obs,nwm)
        if metric == 'FHV':
            lstm_score = fdc_fhv(obs,lstm)
            nwm_score = fdc_fhv(obs,nwm)
        if metric == 'FLV':
            lstm_score = fdc_flv(obs,lstm)
            nwm_score = fdc_flv(obs,nwm)
        if metric == 'Peak-Timing':
            lstm_score = mean_peak_timing(obs,lstm)
            nwm_score = mean_peak_timing(obs,nwm)

        analysis_dict[metric]['lstm'].append(lstm_score)
        analysis_dict[metric]['nwm'].append(nwm_score)
